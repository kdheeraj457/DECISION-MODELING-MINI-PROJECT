...
class Dragon_Cherry_Fest(gym.Env):
    def __init__(self):
        super(Dragon_Cherry_Fest, self).__init__()
        ...
        self.hungry_dDragon_Cherry_Fest = [10, 10]  # Start dragon at the center
        ...
        pygame.display.set_caption('DDragon_Cherry_Fest Cherry Fest: The Dragon_Cherry_Fest\'s Tale')

    def reset(self):
        self.hungry_dDragon_Cherry_Fest = [10, 10]
    ...
    def _get_obs(self):
        ...
        obs[self.hungry_dDragon_Cherry_Fest[0], self.hungry_dDragon_Cherry_Fest[1]] = 1
    ...
    def step(self, action):
        ...
        if action == 0:  # Up
            self.hungry_dDragon_Cherry_Fest[0] = max(0, self.hungry_dDragon_Cherry_Fest[0] - 1)
        elif action == 1:  # Down
            self.hungry_dDragon_Cherry_Fest[0] = min(self.grid_size[0] - 1, self.hungry_dDragon_Cherry_Fest[0] + 1)
        elif action == 2:  # Left
            self.hungry_dDragon_Cherry_Fest[1] = max(0, self.hungry_dDragon_Cherry_Fest[1] - 1)
        elif action == 3:  # Right
            self.hungry_dDragon_Cherry_Fest[1] = min(self.grid_size[1] - 1, self.hungry_dDragon_Cherry_Fest[1] + 1)

        reward = 0
        if self.hungry_dDragon_Cherry_Fest == prev_food_pos:
            reward = 10
            self.food_pos = [random.randint(0, 19), random.randint(0, 19)]
            self.score += 1
            self.steps += 1
        ...
        if self.special_food_pos and self.hungry_dDragon_Cherry_Fest == self.special_food_pos:
            reward = 2
            self.special_food_pos = None
            self.score += 2
        ...
        for enemy_pos in self.enemy_positions:
            if self.hungry_dDragon_Cherry_Fest == enemy_pos:
                reward = -5
    ...
    def render(self, mode='human'):
        ...
        # Blit images
        self.screen.blit(pacman_image, (self.hungry_dDragon_Cherry_Fest[1] * self.cell_size, self.hungry_dDragon_Cherry_Fest[0] * self.cell_size))
    ...
    def move_enemies(self):
        for i, enemy_pos in enumerate(self.enemy_positions):
            target = (
                self.hungry_dDragon_Cherry_Fest[0] + random.randint(-1, 1),
                self.hungry_dDragon_Cherry_Fest[1] + random.randint(-1, 1)
            )
            path = self.bfs(tuple(enemy_pos), target)
            if path:
                self.enemy_positions[i] = list(path[0])
            if self.enemy_positions[i] == self.hungry_dDragon_Cherry_Fest:
                self.penalty_score += 1
                self.steps += 1
